...
public struct UserSearchViewModelActions {
    public init(showDetail: @escaping (UserSearchCellViewModel) -> Void) {
        self.showDetail = showDetail
    }

    let showDetail: (UserSearchCellViewModel) -> Void
}
...
public final class UserSearchViewModel: ObservableObject {
    ...
    private let actions: UserSearchViewModelActions?

    public init(actions: UserSearchViewModelActions? = nil) {
    ...
    func didTapDetail(user: UserSearchCellViewModel) {
        actions?.showDetail(user)
    }
}
